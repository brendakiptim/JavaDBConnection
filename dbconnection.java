...
public class dbconnection {
    ...
    static void createRow(String name, RoleInput role, String password) {
        String command = String.format("insert into users(username, role, password) values('%s','%s','%s')", name, password, role);
...
//                    step 4 output the results
                if (type == commandType.query) {
                    ...
                    while (results.next()) {
                        System.out.println("username is " + results.getString("username") + " role is " + results.getString("role"));
                    }
                    ...
                    while (fetchAll.next()) {
                        System.out.println("username is " + fetchAll.getString("username") + " role is " + fetchAll.getString("role"));
                    }
